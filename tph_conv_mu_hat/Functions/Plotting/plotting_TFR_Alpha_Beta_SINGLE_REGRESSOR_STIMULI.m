...
for i = expgroup
    ...
    ft_sta_stim_left.powspctrm = squeeze(D.fttimelock.powspctrm(stimuli,:,:,:));
    ft_conv_sta_stim{i,1} = ft_sta_stim_left;
...
ft_conv_sta_stim = ft_conv_sta_stim(~cellfun('isempty',ft_conv_sta_stim));
...
% State Anxiety
grandavg_sta_stim = ft_freqgrandaverage(cfg, ft_conv_sta_stim{:});
...
cfg.fontsize = 12;
cfg.zlim = 'maxabs'; % [-5, 5];
cfg.channel = grandavg_sta_stim.label(ChanIndex);
ft_singleplotTFR(cfg,grandavg_sta_stim)
